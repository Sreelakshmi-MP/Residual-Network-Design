...
parser = argparse.ArgumentParser(description='Trains Resnet 18 on CIFAR 10', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--dataset', type=str, default='cifar10', help='Choose Cifar10 dataset')
...
parser.add_argument('--arch', metavar='ARCH', default='resnet18', help='model architecture: ' + ' | ' + ' (default: resnet18)')
# Optimization options
parser.add_argument('--epochs', type=int, default=600, help='Number of epochs to train.')
parser.add_argument('--train', type=str, default = 'mixup_hidden')
...
parser.add_argument('--F', default=3, type=int,
                help='Conv. kernel size in Residual Layer i')
parser.add_argument('--C', default=32, type=int,
                help='# channels in Residual Layer i')
parser.add_argument('--B', default=2, type=int,
                help='# Residual blocks in Residual Layer i')
parser.add_argument('--P', default=4, type=int,
                help='Average pool kernel size')
parser.add_argument('--N', default=4, type=int,
                help='# Residual Layers')
parser.add_argument('--K', default=1, type=int,
                help='Skip connection kernel size in Residual Layer i') 

parser.add_argument('--opt', default='nesterov', type=str,
                help='Optimizer')


args = parser.parse_args()
settings.argsParser = args
...
def experiment_cifar(dataset='cifar10',
                    arch='',
                    epochs=600,
                    dropout='True',
                    batch_size=64,
...
def train(train_loader, model, optimizer, epoch, args, log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.train== 'mixup_hidden':
            ...
            output, target_a, target_b, lam = model(input_var, target_var, mixup_hidden = True,  mixup_alpha = args.mixup_alpha)
...
def main():
    ...
    exp_name=experiment_cifar(dataset=args.dataset,
                    arch=args.arch,
                    epochs=args.epochs,
    ...
    net = project1_model.resnet18(num_classes,args.dropout,per_img_std).cuda() 
    print_log("=> network :\n {}".format(net), log)
    args.num_classes = num_classes
    print('the number of model parameters: {}'.format(sum([p.data.nelement() for p in net.parameters()])))

    if (settings.argsParser.opt == 'sgd'):                               
        # Not adaptive
        optimizer = optim.SGD(net.parameters(), lr=settings.argsParser.lr,
                              momentum=0.9, weight_decay=5e-4)
        print('SGD Optimizer assigned!')
        
    elif (settings.argsParser.opt == 'adam'):
        optimizer = optim.Adam(net.parameters(), lr=settings.argsParser.lr,
                              weight_decay=5e-4)
        print('adam Optimizer assigned!')
        
    elif (settings.argsParser.opt == 'adadelta'):
        optimizer = optim.Adadelta(net.parameters(), lr=settings.argsParser.lr,
                              weight_decay=5e-4)
        print('adadelta Optimizer assigned!')
        
    elif (settings.argsParser.opt == 'nesterov'):
